...
/*	while(choice)
  	{
    	printf("Enter 1 to send message, 2 to receive message, 0 to exit.\n");
    	scanf("%d",&choice);
	    ...
	    if(choice==1)
    	{
      		...
      		scanf("%c",&temp);
      		scanf("%[^\n]s",msg);
      		sendto(server,msg,sizeof(msg),0,(struct sockaddr *)&serverStorage,addr_size);
    	}
    	else if(choice==2)
    	{	
      		len = recvfrom(server,buffer,1024,0,(struct sockaddr *)&serverStorage, &addr_size);
      		printf("Message is : %s\n",buffer);
    	}
    	memset(buffer, 0, sizeof buffer);
    	memset(msg, 0, sizeof msg);
